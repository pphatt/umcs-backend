...
public class ContributionsController : ManagerApiController
{
    ...
    [HttpGet("not-graded-contribution")]
    [Authorize(Permissions.ActivityLogs.View)]
    public async Task<IActionResult> GetUngradedContribution([FromQuery] GetAllUngradedContributionsPaginationRequest request)
    {
        var mapper = _mapper.Map<GetAllUngradedContributionsPaginationQuery>(request);
